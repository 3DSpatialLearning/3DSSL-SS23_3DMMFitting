...
if __name__ == "__main__":
    ...
    outpath = experconfig.outpath
    print("Output path:", outpath)
    log_outpath = Path(outpath) / "log.txt"
    Path(log_outpath).parent.mkdir(parents=True, exist_ok=True)
    log_outpath = str(log_outpath)
    if args.resume:
        iternum = utils.findmaxiters(log_outpath)
    else:
        iternum = 0
    log = utils.Logger(log_outpath, "a" if args.resume else "w")
