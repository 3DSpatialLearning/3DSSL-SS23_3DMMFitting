...
class HairSegmenter:
    ...
    def segment(self, input_image: np.ndarray, visualize: bool = False) -> np.ndarray:
        ...
        assert img_shape_len == 3, "Expected len(img.shape) == 3 (width, height, channel), but found {}.".format(img_shape_len)
        img_channel = input_image.shape[2]
        assert img_channel == 3 or img_channel == 4, "Expected image to have 3 or 4 channels, but found {}".format(img_channel)
